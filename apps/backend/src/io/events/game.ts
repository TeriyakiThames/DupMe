...
export function setupGameEvents(
	 ...
	 */
	socket.on('start-game', async (data: { userProfile: UserProfile }) => {
		try {
	 ...
	 */
	socket.on('save-sequence', async (data: { userProfile: UserProfile; sequence: string[] }) => {
		try {
			if (!data.userProfile || !socket.currentRoomId) {
			...
			// Send sequence to answer player
			if (result.answerPlayerId) {
				// Find the answer player's socket by checking all sockets in room
				const sockets = await io.in(socket.currentRoomId).fetchSockets();
				const answerSocket = sockets.find((s: any) => {
					// Note: We'll need to store userProfile on socket for this to work
					// Or find another way to identify the answer player
					return s.id !== socket.id; // For now, assume other player
				});
			...
			// Notify all other players about sequence being ready
			socket.to(socket.currentRoomId).emit('sequence-ready', {
				roundNumber: result.roundNumber,
				questionPlayer: data.userProfile.username,
				message: `${data.userProfile.username} has created a sequence`
			});
	 ...
	 */
	socket.on('submit-round-result', async (data: { userProfile: UserProfile; pointsEarned: number; success: boolean }) => {
		try {
			if (!data.userProfile || !socket.currentRoomId) {
			...
			if (typeof data.pointsEarned !== 'number' || typeof data.success !== 'boolean') {
			...
			} else if (result.switchedTurns) {
				// Turns switched - notify all players
				io.to(socket.currentRoomId).emit('turns-switched', {
					...result,
					message: `Round ${result.roundNumber}: Roles switched!`
				});
			} else {
	 ...
	 */
	socket.on('end-game', async (data: { userProfile: UserProfile }) => {
		try {
			if (!data.userProfile || !socket.currentRoomId) {
